<?php
/**
 * Shipping Methods Display — theme override.
 *
 * Same as WooCommerce core's own template, with one addition: a short,
 * admin-editable reassurance line after "Shipping to [address]." (Global
 * Settings → Shop — "Cart page — shipping address note"), since that line
 * has no obvious way to act on it otherwise — the destination shown here
 * is a shipping-cost estimate only and is never the address an order
 * actually ships to; checkout collects the real one independently.
 *
 * @package Base Theme
 */
			...
			<?php if ( is_cart() ) : ?>
				<p class="woocommerce-shipping-destination">
					...
					if ( $formatted_destination ) {
						...
						$shipping_note = function_exists( 'myshop_cart_shipping_note' ) ? myshop_cart_shipping_note() : '';
						if ( $shipping_note ) {
							echo '<span class="cart-shipping-note">' . esc_html( $shipping_note ) . '</span>';
						}
					} else {
